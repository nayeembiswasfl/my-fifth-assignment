...
const getIssueLabels = (issue) => {
  const source = issue.labels ?? issue.label ?? issue.category ?? issue.tags ?? 'general';
...
const getLabelBadgeConfig = (label) => {
  ...
  if (normalized.includes('bug')) {
    return {
      tone: 'border-[#fecaca] bg-[#fee2e2] text-[#ef4444]',
      icon: '<img class="w-[12px] h-[12px] object-contain shrink-0" src="./assets/BugDroid.png" alt="" aria-hidden="true" />'
    };
  }

  if (normalized.includes('help wanted')) {
    return {
      tone: 'border-[#fcd34d] bg-[#fef3c7] text-[#d97706]',
      icon: '<img class="w-[12px] h-[12px] object-contain shrink-0" src="./assets/Lifebuoy.png" alt="" aria-hidden="true" />'
    };
  }

  if (normalized.includes('enhancement')) {
    return {
      tone: 'border-[#86efac] bg-[#dcfce7] text-[#059669]',
      icon: '<img class="w-[12px] h-[12px] object-contain shrink-0" src="./assets/Sparkle.png" alt="" aria-hidden="true" />'
    };
  }

  if (normalized.includes('documentation')) {
    return {
      tone: 'border-[#93c5fd] bg-[#dbeafe] text-[#1d4ed8]',
      icon: '<span class="inline-flex w-[12px] h-[12px] rounded-full border border-current items-center justify-center shrink-0"><span class="w-[4px] h-[4px] rounded-full bg-current"></span></span>'
    };
  }

  if (normalized.includes('good first issue')) {
    return {
      tone: 'border-[#d8b4fe] bg-[#f3e8ff] text-[#7e22ce]',
      icon: '<span class="inline-flex w-[12px] h-[12px] rounded-full border border-current items-center justify-center shrink-0"><span class="w-[4px] h-[4px] rounded-full bg-current"></span></span>'
    };
  }

  return {
    tone: 'border-[#cbd5e1] bg-[#e2e8f0] text-[#64748b]',
    icon: '<span class="inline-flex w-[12px] h-[12px] rounded-full border border-current items-center justify-center shrink-0"><span class="w-[4px] h-[4px] rounded-full bg-current"></span></span>'
  };
};

const renderLabelBadges = (issue, limit = 3, compact = false) => {
  const labels = getIssueLabels(issue);
  const list = labels.length ? labels.slice(0, limit) : ['general'];

  const baseClass = compact
    ? 'inline-flex items-center gap-1 px-[8px] py-[3px] rounded-full border text-[10px] font-semibold uppercase tracking-[0.01em] leading-none whitespace-nowrap min-w-0 max-w-[48%] overflow-hidden text-ellipsis'
    : 'inline-flex items-center gap-1 px-[10px] py-1 rounded-full border text-[12px] font-semibold uppercase tracking-[0.01em] leading-none';

  return list
    .map((label) => {
      const badge = getLabelBadgeConfig(label);
      return `<span class="${baseClass} ${badge.tone}">${badge.icon}${escapeHtml(toLabelText(label))}</span>`;
    })
    .join('');
};
...
const renderCounts = (issues) => {
  ...
  if (el.closedCount) {
    el.closedCount.textContent = String(closed);
  }
};
...
const createCard = (issue) => {
  ...
  const statusIconMarkup =
    issue.status === 'closed'
      ? '<img class="w-[21px] h-[21px] object-contain shrink-0" src="./assets/Closed- Status .png" alt="" aria-hidden="true" />'
      : '<img class="w-[21px] h-[21px] object-contain shrink-0" src="./assets/Open-Status.png" alt="" aria-hidden="true" />';
  ...
  card.innerHTML = `
    <div class="p-[13px_13px_12px] min-h-[268px] flex flex-col">
      ...
      <div class="mt-[11px] flex flex-nowrap gap-[6px] overflow-hidden">
        ${renderLabelBadges(issue, 2, true)}
      </div>
...
const fetchIssues = async (options = {}) => {
  ...
  try {
    ...
    const data = extractData(payload).map((issue, idx) => normalizeIssue(issue, idx + 1));
